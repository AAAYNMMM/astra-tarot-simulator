...
星纱塔罗 2.1.0 是一个完全离线运行的中文塔罗牌 GUI。浏览器负责界面、抽牌、规则解读、历史和 PWA；Python 标准库只负责启动本地服务，不需要安装第三方包。

当前开发分支已加入期待契合评估：固定90题分别使用期待评级、现况地图、行动提示或双路径比较。它仍是纯本地规则系统，不调用AI，也不把等级解释为未来发生概率。
...
- Schema 4.0.0精简解读：结论、关键依据、成立前提、停止信号、下一步和可折叠逐牌依据。
- Worker核心预热，当前问题和抽中牌义按需加载；洗牌、发牌和全部翻牌采用有界并行动画。
...
- 552例开发评测、56160例真实数据生成矩阵，以及CWapi受控盲测发布门禁。

## 当前开发分支：期待契合评估

- 可评级问题会在抽牌前要求选择明确期待；无期待、单牌、现况和行动问题不显示`SSS–E`。
- 等级综合整副牌阵的基础、过程、结果契合、稳定性、可控性、负担和证据，不只看结果牌。
- 90个问题保留原ID，但展示文案改为有对象、有时限、单一且可观察的目标。
- 二选一要求填写两条路径并选择判断标准；两边各用独立三牌时间线，不合并总等级、不自动选出赢家。
- 综合页只解释整副牌阵的等级或走势、原因、利弊与现实指引；逐牌资料只留在单牌标签。
- 相同问题和期待在24小时内重复时会先提示上次记录与现实核验，避免把重抽当成改善结果的方法。
...
- 四种牌阵和公开业务 ID 保持固定。
- 抽牌先完成，解读后执行；问题、期待、判断标准和路径名称不会影响抽牌或正逆位，期待只在结构化证据冻结后参与评估。
...
真实发布验证由 CWapi 本地 Runner 对固定 40 位 commit 执行。2.1.0 的任务状态与证据见 [`docs/PROGRESS.md`](docs/PROGRESS.md)。
