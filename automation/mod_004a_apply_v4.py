#!/usr/bin/env python3
"""Apply MOD-004A with current contracts and preserved original updater references."""
...
original_update_python_contract = module.update_python_contract
...
def update_module_contract(source: str) -> str:
    ...
    source = module.replace_once(source, anchor, anchor + "".join(f'  "{path}",\n' for path in required), "module contract required files")
    source = source.replace("astra-tarot-v8", "astra-tarot-v9")
    sw_anchor = '  "data.js",\n  "app.js",\n]) {'
    source = module.replace_once(source, sw_anchor, "".join(f'  "{path}",\n' for path in required[:-1]) + sw_anchor, "module contract SW UI resources")
    source = module.replace_once(source, '["app.js", 1353, "MOD-006A"]', '["app.js", 921, "MOD-006A"]', "reduced app debt assertion")
    return source.replace("MOD-003B module contract passed:", "MOD-004A module contract passed:")


def update_python_contract(source: str) -> str:
    source = original_update_python_contract(source)
