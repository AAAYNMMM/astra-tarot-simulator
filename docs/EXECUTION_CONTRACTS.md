...
### Phase 14：自由问题隔离、统一顺势评分与独立牌阵 V3

Phase 14 由 D-071 至 D-078 约束，取代 Phase 13 的生产新建流程；Phase 13 的 90 题、期待评估和双路径实现降为 V1/V2 历史兼容。固定顺序如下：

1. `V3-001`：冻结`ReadingEnvelopeV3`、严格`EngineReadingRequestV3`和`ReadingPresentationV3`；Worker拒绝问题字段及任何未知字段。
2. `V3-002`：准备页只保留 2–200 Unicode 字符自由问题、四牌阵和四套牌面；删除问题类型、主题、期待、标准、时间与比较入口。
3. `V3-003`：建立四套`SpreadReadingProfile 2.0.0`，Observation、Relation和Claim的新路径不加载`QuestionProfile`或问题领域。
4. `V3-004`：按牌位职责确定性选择逆位机制，保存所选模式；建立牌阵定义 V2、V1 兼容 operator 和关系图。
5. `V3-005`：实现单张牌、时间之流、五牌十字和凯尔特十字四个独立工作流，完整消费各自结构边和阅读顺序。
6. `V3-006`：实现八项综合顺势评分、精确阈值和等级上限；单牌三项不适用且最高 S，不完整输入明确失败。
7. `V3-007`：综合页和逐牌详情接入 V3 合同，所有结论保留证据引用并持续显示问题隔离声明。
8. `V3-008`：接通 rendering 随机流、严格 PRNG V1 重放、ReadingRecord V3、V1/V2/V3 合并读取及双存储删除。
9. `V3-009`：重建 artifact/precache，执行 V3 定向测试、旧引擎兼容矩阵、浏览器四牌阵、离线重开、generated 与 full 验证。
10. `REL-007`：后移到 Phase 14 功能与文案冻结之后；另行确定正式版本和最终 exact commit，本地工作区验证不得冒充发布完成。

阶段不变量：

- 自由问题只进入标题与历史；不得进入随机、Worker、Observation、Relation、Claim、Assessment、Presentation、artifact 消费指纹或诊断。
- 相同 reading ID、牌阵版本、牌面和随机审计下，任意问题正文产生字节级相同的 V3 请求与引擎结果。
- 四牌阵全部显示“综合顺势等级”，但等级不得解释为问题答案、发生概率或愿望达成。
- 四个工作流不共享通用综合模板；综合页不显示牌名或逐牌基础牌义，逐牌详情不缺失当前方向牌义和牌阵作用。
- 随机算法和逆位概率保持`astra-prng-v1`兼容；未知算法或版本只读，不重放。
- V1/V2历史不迁移、不重评分、不重解读；凯尔特 V1 的`advice`与 V2 的`self`按各自定义展示。

阶段出口：V3 隔离合同、四牌阵黄金测试、八项评分边界、随机黄金向量、历史混合读取、Chrome/Edge 四牌阵、离线重开、生成物和`test:full`全部通过。正式发布仍需另行在最终 commit 执行`REL-007`。
