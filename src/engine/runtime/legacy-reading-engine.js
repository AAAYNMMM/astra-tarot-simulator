...
import { LEGACY_SPREADS_V1 } from "../../knowledge/spreads/definitions.js";
...
export async function executeLegacyReadingEngine({
  questionId,
  spreadId,
  draws,
  ...
  const spread = LEGACY_SPREADS_V1.find((item) => item.id === spreadId);
  ...
  const observations = normalized.map((draw, index) => {
    ...
    const reversalMode = draw.orientation === "reversed" ? card.reversal.supportedModes[0] : null;
    return createObservation({
      card,
      question,
      operator: getPositionOperator(spreadId, draw.position.id, "1.0.0"),
      orientation: draw.orientation,
      reversalMode,
    });
  });
  const structuralBatch = createStructuralRelationCandidates({
    spreadId,
    observations,
  ...
  const rendered = renderReadingText({ claim: claimPipeline.claim, candidateBatch: claimPipeline.candidateBatch, observations, renderingStream });
  return deepFreeze({
    schemaVersion: "1.0.0",
    questionId,
    spreadId,
